...
getObj: function( bucketname, objname, path, callback)
{
	var s3 = new AWS.S3();

	var filePath = fs.createWriteStream(path + objname);
	...
	send(
		...
		{
			if (err) {
				console.log("Get "+objname+" fail!!!"/*err, err.stack*/); // an error occurred
				fs.unlink(path + objname);
			}
